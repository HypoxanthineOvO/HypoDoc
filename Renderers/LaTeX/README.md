...
[![HypoDoc Markdown](https://img.shields.io/badge/HypoDoc%20Markdown-0.2.0--rc.1-7c3aed)](../../Spec/spec/hypodoc-markdown.md)
...
HypoDoc 是文档格式家族。本目录是 HypoDoc Monorepo 中的 LaTeX/PDF 渲染实现：

- **HypoDoc Markdown**：AI-first 的结构化 Markdown 源格式。
- **HypoDoc Spec**：独立的格式规范，作为根 `Spec/` submodule 固定兼容 revision 引入。
- **HypoDoc LaTeX Renderer**：本目录，包含 `hypolatex` CLI、Pandoc Lua filter、LaTeX 宏包、主题、模板、测试和 AI Skill。
...
- Beamer/slides 已支持当前 Slides DSL，但不包含自动 Marp/任意 LaTeX deck converter。

...
def _markdown_resource_references(source: Path) -> tuple[str, ...]:
    ...
    for reference in _frontmatter_image_references(document):
        if reference and reference not in seen:
            seen.add(reference)
            references.append(reference)
...
_FRONTMATTER_IMAGE_KEYS = ("logo", "logolong", "logonegative", "cover_image")


def _frontmatter_image_references(document: object) -> Iterable[str]:
    """Yield image paths stored in frontmatter metadata keys.

    The AST walk only finds in-body figures and images; cover and logo images
    live in YAML metadata and must be copied into the build directory too.
    """
    ...
    if not isinstance(meta, dict):
        return
    for key in _FRONTMATTER_IMAGE_KEYS:
        value = meta.get(key)
        if value is None:
            continue
...
def _meta_text(node: object) -> str:
    """Flatten a pandoc metadata value to plain text."""
    ...
    if isinstance(node, dict):
        ...
        if isinstance(contents, str):
            return contents
        return _meta_text(contents)
    if isinstance(node, list):
        return "".join(_meta_text(item) for item in node)
    return ""

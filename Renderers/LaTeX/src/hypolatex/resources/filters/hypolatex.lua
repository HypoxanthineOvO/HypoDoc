...
local function table_config(div, yaml_config)
  ...
  config.rowheaders = parse_bool(config.rowheaders, false)
...
local function render_tblr_table(config, columns, head_rows, body_rows)
  ...
  if config.header and #head_rows > 0 then
    options[#options + 1] = "row{1}={bg=HypoTableHead,fg=HypoTableHeadFg,font=\\bfseries}"
    options[#options + 1] = "hline{2}={0.04em}"
  end
  if config.rowheaders then
    options[#options + 1] = "column{1}={bg=HypoTableRowHead,font=\\bfseries}"
  end

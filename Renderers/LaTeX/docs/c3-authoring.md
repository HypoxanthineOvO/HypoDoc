...
`label`, `header`, `striped`, `rowheaders`, and `columns`. In `columns`, use
`align`, `width`, and `weight` instead of raw LaTeX column specifications.
With `header` on (the default), the header row renders as an accent color
block with white bold text; `rowheaders: true` additionally tints the first
column and sets it bold for tables whose first column holds row labels.

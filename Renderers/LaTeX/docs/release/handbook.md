...
`hypodoc-spec` 是 HypoDoc Markdown 草案的权威来源。本仓库中的根 `Spec/` 是同步进入项目的规范草案来源。本手册是解释性和发布同步章节，用来说明 Hypo-LaTeX 当前支持的公开写作方式；它不是规范真相源。

当手册说明和 `hypodoc-spec` 或根 `Spec/` 发生差异时，应以规范草案为准，并把本手册更新为同步解释，而不是在手册中另建一套规范。
...
不建议在发布文档中直接写复杂 LaTeX 宏、不可移植字体路径或依赖本机私有素材的图片引用。需要表达复杂版式时，优先查 `hypodoc-spec` 和根 `Spec/`，再同步到 Hypo-LaTeX 已支持的子集。
...
- 手册只解释当前实现和同步章节，不取代 `hypodoc-spec` 或根 `Spec/`。

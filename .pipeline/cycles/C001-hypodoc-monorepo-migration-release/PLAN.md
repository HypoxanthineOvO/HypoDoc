...
- 不把 Hypo-Markdown 或 Hypo-LaTeX 的 Git 历史合并进新仓 `main`；旧仓、来源 SHA、tag、
  dirty patch 与验证结果作为 provenance 保留，筛选后的内容直接进入新的产品提交历史。
...
- Hypo-Markdown 从固定来源 SHA 筛选内容，直接整理到 `Packages/`、`Apps/Desktop` 与
  `Extensions/VSCode`；新提交按真实产品改动分层，不连接旧仓 commit DAG。
- Hypo-LaTeX 从固定来源 SHA 直接整理到 `Renderers/LaTeX`；旧仓独立归档，通用写作规则提取到
...
- V4 Flash 可作为 M2 至 M4 的主执行模型；交接必须包含当前 Cycle 记录、inventory 与决策清单，
  并要求它在 `S2` 停止等待人工审阅。
...
| `S1` | 迁移基线与业务整合设计审阅 | 用户看到真实 inventory、权威层级、直接迁入映射、拟修改内容、许可证结论和风险清单，并决定是否允许开始导入 | 接受标准：每个来源可恢复，干净产品历史和业务改造范围清楚，dirty 文件处置明确，MIT 与 Spec 固定/修订方式无未决阻塞 |
...
| `M3` | 整合 Spec、LaTeX 与 Skills | 根 Spec 固定独立 revision；LaTeX 内容以干净产品提交进入 renderer；通用与专用 Skill 分离；重复 Spec 引用消除 | 来源 SHA/迁移清单与依赖检查、Spec pytest/registry check、LaTeX pytest/CLI doctor、代表性 TeX/PDF 构建与 PDF 检查 |
...
| `S2` | Release Candidate 与发布治理审阅 | 用户看到 `v0.1.0` RC 产物、兼容性报告、canonical remote、mirror、publisher 与签名状态 | 接受标准：`v0.1.0` 内容与渠道明确，阻塞平台不被误报，远端副作用和回滚方案获准 |
| `M5` | 发布 `v0.1.0` | 从已接受 RC 产生 tag、Release、支持平台资产、VSIX、checksums 与镜像同步记录 | 远端 tag/commit/asset 对照、安装 smoke、checksum 验证和发布日志 |

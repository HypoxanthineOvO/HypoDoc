...
4. Copy, create, or edit a Markdown template before building. Use `templates/longform.md` as the longform starting point, `templates/beamer.md` as the Beamer slides starting point, or make a similar `.md` template with minimal frontmatter and supported fenced directives.
...
- `shanghaitech`: Beamer-only campus theme (university red `#8B0000`). Adds selectable `cover_layout` covers (`cover-e` slanted-block default, `cover-b` rounded band, `cover-c` strip + band + motto), floating-card section dividers, and the classic band frame title. Supports optional `logo`, `logonegative`, and `logolong` frontmatter images (transparent background required); has no longform variant.
- `minimal`: Beamer-only Swiss flat theme: monochrome canvas, ink serif type, hairline rules, flat gray block panels. Use for content-first technical decks.
- `glass`: Beamer-only light-gradient glassmorphism theme: subtle palette-tinted gradient canvas, translucent glass panels with real opacity, white serif type (Noto Serif CJK + TeX Gyre Termes). Compact cover card at the top with authors below on the canvas.
...
Start from `templates/beamer.md` for a public function matrix deck:

```bash
hypolatex build templates/beamer.md --output build/skill-beamer.pdf
```
...
```bash
hypolatex build templates/review.md --answer-mode review --output build/review.pdf
```
...
uv run hypolatex build templates/project.md --theme tech-minimal --output build/c3-skill-project.pdf
uv run hypolatex build templates/review.md --answer-mode review --output build/c3-skill-review.pdf
```
